...
inline Color operator+(const Color& c1, const Color& c2) {
    return Color(std::clamp(static_cast<float>(c1.r) + static_cast<float>(c2.r), 0.f, 255.f),
                 std::clamp(static_cast<float>(c1.g) + static_cast<float>(c2.g), 0.f, 255.f),
                 std::clamp(static_cast<float>(c1.b) + static_cast<float>(c2.b), 0.f, 255.f), static_cast<float>(c1.a));
}

inline Color operator*(float coeff, const Color& c) {
    return Color(std::clamp(static_cast<float>(c.r) * coeff, 0.f, 255.f),
                 std::clamp(static_cast<float>(c.g) * coeff, 0.f, 255.f),
                 std::clamp(static_cast<float>(c.b) * coeff, 0.f, 255.f), static_cast<float>(c.a));
}
...
void ImageBuffer<T>::Set(unsigned int w, unsigned int h, T c) {
    if (!(!canvas || w >= width || h >= height)) this->canvas[(size_t) (h * this->width + w)] = c;
}

template <typename T>
std::optional<T> ImageBuffer<T>::Get(unsigned int w, unsigned int h) const {
    if (!(!canvas || w >= width || h >= height)) return this->canvas[(size_t) (h * this->width + w)];
    return std::nullopt;
}
